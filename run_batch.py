...
BOUNDS = {
    ...
    'E_des': (0.6,  1.4),   # eV, metallic binding energies (lowered to allow desorption!)
...
def run_one(i, n_samples_total, params, max_steps=KMC_MAX_STEPS, n=100):
    print(f"Sim {i+1:>3}/{n_samples_total} | F={params['F']:.4f} E_d={params['E_d']:.2f} " 
...
if __name__ == '__main__':
    ...
    param_list, keys = latin_hypercube_sample(n_samples, BOUNDS)

    with parallel_backend('loky'):
        results = Parallel(n_jobs=-1, verbose=1)(
            delayed(run_one)(i, n_samples, p) for i, p in enumerate(param_list)
        )
